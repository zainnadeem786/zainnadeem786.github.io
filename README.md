...
Verified contact links are maintained in `index.html`. If a public profile changes, update its visible link, the homepage JSON-LD `sameAs` array, and any matching social metadata together. Keep private contact details out of structured data unless there is a clear reason to publish them there.
...
Add `https://zainnadeem786.github.io/` as a URL-prefix property in [Google Search Console](https://search.google.com/search-console/), complete verification, then submit `sitemap.xml` under **Sitemaps**. After meaningful page changes, use URL Inspection for the affected canonical URL and request indexing once. Keep each page's title, description, canonical URL, Open Graph URL, and sitemap entry aligned.
